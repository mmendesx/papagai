...
@Component({
        ...
        <div class="app-form">
          ...
          <div class="app-form__section">
            ...
            <div style="display: flex; gap: 0.75rem;">
              <label
                class="scope-option"
                [class.scope-option--active]="scope() === 'account'"
                (click)="scope.set('account')"
              >
          ...
          @if (scope() === 'account') {
            @if (templatesRes.isLoading()) {
              <div
                class="ec-skeleton"
                style="height: 2.5rem; border-radius: var(--radius-md);"
              ></div>
            } @else if (templatesRes.status() === ResourceStatus.Error) {
              <p class="instance-msg instance-msg--error">
                Erro ao carregar templates de permissao
              </p>
            } @else if (templatesRes.value().templates.length === 0) {
              <p class="instance-msg">
                Nenhum template disponivel. A chave sera criada com acesso
                total.
              </p>
            } @else {
              <tui-textfield>
  ...
  styles: [
    `
      ...
      .scope-option:not(.scope-option--active):not(
...
export class CreateApiKeyDialogComponent {
  ...
  readonly context =
    injectContext<
      TuiDialogContext<CreateApiKeyResult, CreateApiKeyDialogData>
    >();
  ...
  readonly templatesRes = httpResource<{
    templates: ApiKeyPermissionTemplate[];
  }>(
    () => {
      return this.scope() === 'account'
        ? '/api/auth/apikeys/templates'
        : undefined;
    },
    {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      const preferredTemplate =
        templates.find((template) => template.id === 'instance_manager') ??
        templates[0];
  ...
  async submit(): Promise<void> {
    ...
    const request$ =
      this.scope() === 'instance'
        ? this.apiKeysService.createInstanceKey(
            this.selectedInstance()!.name,
            payload,
          )
