    ...
    <div class="settings-body" [@staggerSections]>
      ...
      >
        <div class="section-header">
          ...
          <span
            class="webhook-badge"
            [class.webhook-badge--active]="whEnabled()"
          >
            {{ whEnabled() ? 'ativo' : 'inativo' }}
          </span>
        </div>
        <p class="section-desc">
          Configure a URL e os eventos que serão enviados para o seu servidor.
        </p>
        ...
        <form
          class="webhook-form app-form"
          (ngSubmit)="saveWebhook()"
          aria-label="Formulário de webhook"
        ...
        >
          <label class="toggle-row">
            <input
              ...
              id="webhook-enabled"
            />
            <span class="toggle-label" id="webhook-enabled-label"
              >Ativar webhook</span
            >
          </label>
          ...
          <fieldset class="events-fieldset">
            <legend class="events-legend app-form__section-title">
              Eventos
            </legend>
      ...
      @if (status()?.provider === 'wba' && status()?.wba; as wba) {
        <section
          class="settings-section"
          aria-labelledby="wba-heading"
          [@fadeInUp]
        >
          <div class="section-header">
            <h2 class="section-heading" id="wba-heading">WBA setup</h2>
          </div>
          <p class="section-desc">
          ...
          <ul class="wba-list">
            <li>
              <strong>Phone Number ID:</strong> {{ wba.phoneNumberId || '—' }}
            </li>
            <li>
              <strong>Business Account ID:</strong>
              {{ wba.businessAccountId || '—' }}
            </li>
            <li>
              <strong>Display Number:</strong>
              {{ wba.displayPhoneNumber || '—' }}
            </li>
            <li>
              <strong>Webhook configured at:</strong>
              {{ wba.webhookConfiguredAt || 'not configured' }}
            </li>
            <li>
              <strong>Last health check:</strong>
              {{ wba.lastHealthCheckStatus || 'unknown' }}
            </li>
          </ul>
        </section>
      }
      ...
      >
        <div class="section-header">
          <h2 class="section-heading danger-heading" id="danger-heading">
            Zona de risco
          </h2>
        </div>
        ...
        <div class="danger-action">
          <div class="danger-action-body">
            ...
            <span class="danger-action-desc"
              >Remove permanentemente esta instância e todos os seus
              dados.</span
            >
          </div>
  ...
  async saveWebhook(): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        this.alerts
          .open('Headers devem ser um objeto JSON.', {
            label: 'Erro',
            appearance: 'negative',
            autoClose: 4000,
          })
      ...
      this.alerts
        .open('Headers devem ser um JSON válido.', {
          label: 'Erro',
          appearance: 'negative',
          autoClose: 4000,
        })
    ...
    try {
      ...
      this.alerts
        .open('Configurações de webhook salvas.', {
          label: 'Feito',
          appearance: 'positive',
          autoClose: 3000,
        })
